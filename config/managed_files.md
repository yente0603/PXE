...
- `absent/`  
  Stores markers for managed paths that did not exist before the first
  installation. The corresponding paths are removed during uninstallation.

- `apache/`  
  Stores the original enabled or disabled state of managed Apache sites and
  modules.
...
- `managed_files.list`  
  Lists the filesystem paths managed by the installer. These paths are checked
  when recording and restoring the original system state.

- `recorded`  
  A marker file indicating that the original system state has already been
  recorded. Later installations or upgrades must not overwrite the recorded
  state while this marker exists.

## Example File Tree

The exact contents depend on the state of the system before the **first**
installation.
